...
import com.ump.impl.JdbcPlayer_activity_battleDAO;
import com.ump.model.DBCache;
import com.ump.model.Player_activity_battle;

public class RW_Player_activity_battleTableService extends UntypedActor {
	protected final Log logger = LogFactory.getLog(getClass());

	public static Props props(DataSource dataSource, String DBName) {
		return Props.create(RW_Player_activity_battleTableService.class, dataSource, DBName);
	}

	private static HashMap<Integer, Player_activity_battle> hmCache = new HashMap<Integer, Player_activity_battle>();
	private ActorRef selectWorker;
	private ActorRef insertUpdateWorker;

	private String tableName = "player_activity_battle";
	...
	public RW_Player_activity_battleTableService(DataSource dataSource, String DBName) {
		this.DBName = DBName;
	}
	...
	public void onReceive(Object msg) throws Exception {
		if (msg instanceof Player_activity_battle) {
			Player_activity_battle pg = (Player_activity_battle) msg;
			if (pg.getAction() == DBCache.SELECT) {
				...
				Iterator<Map.Entry<Integer, Player_activity_battle>> iterator = hmCache.entrySet().iterator();
				while (iterator.hasNext()) {
					Map.Entry<Integer, Player_activity_battle> entry = iterator.next();
					Player_activity_battle state = entry.getValue();
	...
	public static void addCache(Player_activity_battle pg) {
	...
	public static class PlayerGoldSelectWorker extends UntypedActor {
		...
		public void onReceive(Object msg) throws Exception {
			if (msg instanceof Integer) {// 收到请求角色消息
				...
				Player_activity_battle pg = hmCache.get(id);
				if (pg == null) {
					pg = JdbcPlayer_activity_battleDAO.getInstance().getPlayer_activity_battle(id.intValue());
					if (pg == null) {
						pg = new Player_activity_battle();
					} else {
						addCache(pg);
					}
				}
	...
	public static class PlayerGoldInsertUpDateWorker extends UntypedActor {
		private HashMap<Integer, Player_activity_battle> hmInsert = new HashMap<Integer, Player_activity_battle>();
		private HashMap<Integer, Player_activity_battle> hmUpDate = new HashMap<Integer, Player_activity_battle>();
		private Cancellable heart;// 心跳 清理缓存中超时的
		...
		@Override
		public void onReceive(Object msg) throws Exception {
			if (msg instanceof Player_activity_battle) {// 收到请求角色消息
				Player_activity_battle pg = (Player_activity_battle) msg;
				...
				if (len > 0) {
					Player_activity_battle[] pgs = new Player_activity_battle[len];
					hmInsert.values().toArray(pgs);
					JdbcPlayer_activity_battleDAO.getInstance().addList(pgs);
					hmInsert.clear();
				}
				...
				if (len > 0) {
					...
					JdbcPlayer_activity_battleDAO.getInstance().upDateList(pgs);
					hmUpDate.clear();
				}
			}
		}
