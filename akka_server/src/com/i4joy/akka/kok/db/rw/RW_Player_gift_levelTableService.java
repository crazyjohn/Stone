...
import com.ump.impl.JdbcPlayer_gift_levelDAO;
import com.ump.model.DBCache;
import com.ump.model.Player_gift_level;

public class RW_Player_gift_levelTableService extends UntypedActor {
	protected final Log logger = LogFactory.getLog(getClass());

	public static Props props(DataSource dataSource, String DBName) {
		return Props.create(RW_Player_gift_levelTableService.class, dataSource, DBName);
	}

	private static HashMap<Integer, Player_gift_level> hmCache = new HashMap<Integer, Player_gift_level>();
	private ActorRef selectWorker;
	private ActorRef insertUpdateWorker;

	private String tableName = "player_gift_level";
	...
	public RW_Player_gift_levelTableService(DataSource dataSource, String DBName) {
		this.DBName = DBName;
	}
	...
	public void onReceive(Object msg) throws Exception {
		if (msg instanceof Player_gift_level) {
			Player_gift_level pg = (Player_gift_level) msg;
			if (pg.getAction() == DBCache.SELECT) {
				...
				Iterator<Map.Entry<Integer, Player_gift_level>> iterator = hmCache.entrySet().iterator();
				while (iterator.hasNext()) {
					Map.Entry<Integer, Player_gift_level> entry = iterator.next();
					Player_gift_level state = entry.getValue();
	...
	public static void addCache(Player_gift_level pg) {
	...
	public static class PlayerGoldSelectWorker extends UntypedActor {
		...
		public void onReceive(Object msg) throws Exception {
			if (msg instanceof Integer) {// 收到请求角色消息
				...
				Player_gift_level pg = hmCache.get(id);
				if (pg == null) {
					pg = JdbcPlayer_gift_levelDAO.getInstance().getPlayer_gift_level(id.intValue());
					if (pg == null) {
						pg = new Player_gift_level();
					} else {
						addCache(pg);
					}
				}
	...
	public static class PlayerGoldInsertUpDateWorker extends UntypedActor {
		private HashMap<Integer, Player_gift_level> hmInsert = new HashMap<Integer, Player_gift_level>();
		private HashMap<Integer, Player_gift_level> hmUpDate = new HashMap<Integer, Player_gift_level>();
		private Cancellable heart;// 心跳 清理缓存中超时的
		...
		@Override
		public void onReceive(Object msg) throws Exception {
			if (msg instanceof Player_gift_level) {// 收到请求角色消息
				Player_gift_level pg = (Player_gift_level) msg;
				...
				if (len > 0) {
					Player_gift_level[] pgs = new Player_gift_level[len];
					hmInsert.values().toArray(pgs);
					JdbcPlayer_gift_levelDAO.getInstance().addList(pgs);
					hmInsert.clear();
				}
				...
				if (len > 0) {
					...
					JdbcPlayer_gift_levelDAO.getInstance().upDateList(pgs);
					hmUpDate.clear();
				}
			}
		}
