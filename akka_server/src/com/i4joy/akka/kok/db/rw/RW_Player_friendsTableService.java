...
import com.ump.impl.JdbcPlayer_friendsDAO;
import com.ump.model.DBCache;
import com.ump.model.Player_friends;

public class RW_Player_friendsTableService extends UntypedActor {
	protected final Log logger = LogFactory.getLog(getClass());

	public static Props props(DataSource dataSource, String DBName) {
		return Props.create(RW_Player_friendsTableService.class, dataSource, DBName);
	}

	private static HashMap<Integer, Player_friends> hmCache = new HashMap<Integer, Player_friends>();
	private ActorRef selectWorker;
	private ActorRef insertUpdateWorker;

	private String tableName = "player_friends";
	...
	public RW_Player_friendsTableService(DataSource dataSource, String DBName) {
		this.DBName = DBName;
	}
	...
	public void onReceive(Object msg) throws Exception {
		if (msg instanceof Player_friends) {
			Player_friends pg = (Player_friends) msg;
			if (pg.getAction() == DBCache.SELECT) {
				...
				Iterator<Map.Entry<Integer, Player_friends>> iterator = hmCache.entrySet().iterator();
				while (iterator.hasNext()) {
					Map.Entry<Integer, Player_friends> entry = iterator.next();
					Player_friends state = entry.getValue();
	...
	public static void addCache(Player_friends pg) {
	...
	public static class PlayerGoldSelectWorker extends UntypedActor {
		...
		public void onReceive(Object msg) throws Exception {
			if (msg instanceof Integer) {// 收到请求角色消息
				...
				Player_friends pg = hmCache.get(id);
				if (pg == null) {
					pg = JdbcPlayer_friendsDAO.getInstance().getPlayer_friends(id.intValue());
					if (pg == null) {
						pg = new Player_friends();
					} else {
						addCache(pg);
					}
				}
	...
	public static class PlayerGoldInsertUpDateWorker extends UntypedActor {
		private HashMap<Integer, Player_friends> hmInsert = new HashMap<Integer, Player_friends>();
		private HashMap<Integer, Player_friends> hmUpDate = new HashMap<Integer, Player_friends>();
		private Cancellable heart;// 心跳 清理缓存中超时的
		...
		@Override
		public void onReceive(Object msg) throws Exception {
			if (msg instanceof Player_friends) {// 收到请求角色消息
				Player_friends pg = (Player_friends) msg;
				...
				if (len > 0) {
					Player_friends[] pgs = new Player_friends[len];
					hmInsert.values().toArray(pgs);
					JdbcPlayer_friendsDAO.getInstance().addList(pgs);
					hmInsert.clear();
				}
				...
				if (len > 0) {
					...
					JdbcPlayer_friendsDAO.getInstance().upDateList(pgs);
					hmUpDate.clear();
				}
			}
		}
