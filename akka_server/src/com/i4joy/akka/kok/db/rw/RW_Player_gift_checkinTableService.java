...
import com.ump.impl.JdbcPlayer_gift_checkinDAO;
import com.ump.model.DBCache;
import com.ump.model.Player_gift_checkin;

public class RW_Player_gift_checkinTableService extends UntypedActor {
	protected final Log logger = LogFactory.getLog(getClass());

	public static Props props(DataSource dataSource, String DBName) {
		return Props.create(RW_Player_gift_checkinTableService.class, dataSource, DBName);
	}

	private static HashMap<Integer, Player_gift_checkin> hmCache = new HashMap<Integer, Player_gift_checkin>();
	private ActorRef selectWorker;
	private ActorRef insertUpdateWorker;

	private String tableName = "player_gift_checkin";
	...
	public RW_Player_gift_checkinTableService(DataSource dataSource, String DBName) {
		this.DBName = DBName;
	}
	...
	public void onReceive(Object msg) throws Exception {
		if (msg instanceof Player_gift_checkin) {
			Player_gift_checkin pg = (Player_gift_checkin) msg;
			if (pg.getAction() == DBCache.SELECT) {
				...
				Iterator<Map.Entry<Integer, Player_gift_checkin>> iterator = hmCache.entrySet().iterator();
				while (iterator.hasNext()) {
					Map.Entry<Integer, Player_gift_checkin> entry = iterator.next();
					Player_gift_checkin state = entry.getValue();
	...
	public static void addCache(Player_gift_checkin pg) {
	...
	public static class PlayerGoldSelectWorker extends UntypedActor {
		...
		public void onReceive(Object msg) throws Exception {
			if (msg instanceof Integer) {// 收到请求角色消息
				...
				Player_gift_checkin pg = hmCache.get(id);
				if (pg == null) {
					pg = JdbcPlayer_gift_checkinDAO.getInstance().getPlayer_gift_checkin(id.intValue());
					if (pg == null) {
						pg = new Player_gift_checkin();
					} else {
						addCache(pg);
					}
				}
	...
	public static class PlayerGoldInsertUpDateWorker extends UntypedActor {
		private HashMap<Integer, Player_gift_checkin> hmInsert = new HashMap<Integer, Player_gift_checkin>();
		private HashMap<Integer, Player_gift_checkin> hmUpDate = new HashMap<Integer, Player_gift_checkin>();
		private Cancellable heart;// 心跳 清理缓存中超时的
		...
		@Override
		public void onReceive(Object msg) throws Exception {
			if (msg instanceof Player_gift_checkin) {// 收到请求角色消息
				Player_gift_checkin pg = (Player_gift_checkin) msg;
				...
				if (len > 0) {
					Player_gift_checkin[] pgs = new Player_gift_checkin[len];
					hmInsert.values().toArray(pgs);
					JdbcPlayer_gift_checkinDAO.getInstance().addList(pgs);
					hmInsert.clear();
				}
				...
				if (len > 0) {
					...
					JdbcPlayer_gift_checkinDAO.getInstance().upDateList(pgs);
					hmUpDate.clear();
				}
			}
		}
