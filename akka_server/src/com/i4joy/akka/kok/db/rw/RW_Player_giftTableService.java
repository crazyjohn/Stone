...
import com.ump.impl.JdbcPlayer_giftDAO;
import com.ump.model.DBCache;
import com.ump.model.Player_gift;

public class RW_Player_giftTableService extends UntypedActor {
	protected final Log logger = LogFactory.getLog(getClass());

	public static Props props(DataSource dataSource, String DBName) {
		return Props.create(RW_Player_giftTableService.class, dataSource, DBName);
	}

	private static HashMap<Integer, Player_gift> hmCache = new HashMap<Integer, Player_gift>();
	private ActorRef selectWorker;
	private ActorRef insertUpdateWorker;

	private String tableName = "player_gift";
	...
	public RW_Player_giftTableService(DataSource dataSource, String DBName) {
		this.DBName = DBName;
	}
	...
	public void onReceive(Object msg) throws Exception {
		if (msg instanceof Player_gift) {
			Player_gift pg = (Player_gift) msg;
			if (pg.getAction() == DBCache.SELECT) {
				...
				Iterator<Map.Entry<Integer, Player_gift>> iterator = hmCache.entrySet().iterator();
				while (iterator.hasNext()) {
					Map.Entry<Integer, Player_gift> entry = iterator.next();
					Player_gift state = entry.getValue();
	...
	public static void addCache(Player_gift pg) {
	...
	public static class PlayerGoldSelectWorker extends UntypedActor {
		...
		public void onReceive(Object msg) throws Exception {
			if (msg instanceof Integer) {// 收到请求角色消息
				...
				Player_gift pg = hmCache.get(id);
				if (pg == null) {
					pg = JdbcPlayer_giftDAO.getInstance().getPlayer_gift(id.intValue());
					if (pg == null) {
						pg = new Player_gift();
					} else {
						addCache(pg);
	...
	public static class PlayerGoldInsertUpDateWorker extends UntypedActor {
		private HashMap<Integer, Player_gift> hmInsert = new HashMap<Integer, Player_gift>();
		private HashMap<Integer, Player_gift> hmUpDate = new HashMap<Integer, Player_gift>();
		private Cancellable heart;// 心跳 清理缓存中超时的
		...
		@Override
		public void onReceive(Object msg) throws Exception {
			if (msg instanceof Player_gift) {// 收到请求角色消息
				Player_gift pg = (Player_gift) msg;
				...
				if (len > 0) {
					Player_gift[] pgs = new Player_gift[len];
					hmInsert.values().toArray(pgs);
					JdbcPlayer_giftDAO.getInstance().addList(pgs);
					hmInsert.clear();
				}
				...
				if (len > 0) {
					...
					JdbcPlayer_giftDAO.getInstance().upDateList(pgs);
					hmUpDate.clear();
				}
			}
		}
