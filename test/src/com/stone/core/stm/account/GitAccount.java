 ...
 * java软件事务内存的实现(ACID); A: atomicity 原子性;<br>
 * C: consistent 一致性；<br>
 * I: isolated 隔离性；<br>
 * 
 * @author crazyjohn
...
public class GitAccount {
	...
	public GitAccount(final long amount) {
		atomic(new Runnable() {
			@Override
			public void run() {
	...
	public static void main(String[] args) {
		// create from and to
		final GitAccount from = new GitAccount(10);
		final GitAccount to = new GitAccount(20);
		// do tansfer
		...
		atomic(new Runnable() {
			@Override
			public void run() {
				System.out.println("After transfer - From: " + from.amount.get());
				System.out.println("After transfer - To: " + to.amount.get());
			}
